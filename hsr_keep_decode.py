...
# @Last Modified time: 2019-07-08 12:08:14
...
def nfold_hsr_arg(input_file, nfold=5, word_feature="FF", use_char = False, lr= 0.2, cutoff=5, pred="HSR"):
    ...
    for idx in range(nfold):
        ...
        ncrf.data.words2sent_representation = "MAX"
        if pred == "ADR":
    ...
    plot_precision_recall(precision, recall, model_name, "keepdecode/"+model_name+".prc.jpg")
    plot_roc(fpr, tpr, auc, model_name, "keepdecode/"+model_name+".roc.jpg")
    write_results("keepdecode/"+model_name + ".decode.txt", [test_all_gold, test_all_target_decode], ["Gold Label", "Predict Prob"])
    write_results("keepdecode/"+model_name+".result.txt", [precision, recall, fpr, tpr], ["Precision","Recall", "FPR", "TPR"], auc)
    print("Auc:", auc)
    fout = open("keepdecode/all_results.txt", 'a')
